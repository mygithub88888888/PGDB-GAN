...
def generate_face_mask(image_size, bbox_list):
    """"""
...
def load_face_mask(img_name, mask_dir, image_size):
    """"""
    mask_path = os.path.join(mask_dir, img_name.replace('.png', '_mask.npy'))
    if os.path.exists(mask_path):
        mask = np.load(mask_path)
    else:
        mask = np.zeros(image_size, dtype=np.uint8)
    mask = transforms.ToTensor()(mask).unsqueeze(0).float()
    return mask
