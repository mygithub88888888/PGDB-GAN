...
def local_mean(image, patch_size=5):
...
class LossFunction(nn.Module):
    ...
    def forward(self, input, s2, H3, H2_blur, H3_blur, discriminator, face_mask=None):
        loss = 0

        L11, L12 = pair_downsampler(input)
        loss += self._l2_loss(s2, torch.clamp(s2, 1e-9, 1)) * 700

        loss += self.smooth_loss(input, s2) * 5

        loss += self.L_TV_loss(s2) * 1600

        d_real = discriminator(input.detach())
        d_fake = discriminator(H3.detach())
        loss_d = self.gan_loss(d_real, True) + self.gan_loss(d_fake, False)
        loss_g = self.gan_loss(discriminator(H3), True)

        loss_region = 0
        if face_mask is not None:
            masked_H3 = H3 * face_mask
            masked_input = input * face_mask
            loss_region = self._l1_loss(masked_H3, masked_input) * 10

        total_loss = loss + loss_g * 0.1 + loss_d * 0.1 + loss_region
        return total_loss